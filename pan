...
OUTPUT_XLSX  = "applogs_pan.xlsx"
OUTPUT_DOCX  = "applogs_pan.docx"

# PAN regex: no A–Z or digit immediately before/after
...
def main():
    ...
    for line_no, line in tqdm(logs, desc="Parsing logs"):
        ...
        for field, val in data.items():
            for m in PAN_REGEX.finditer(str(val)):
                parsed.append({
                    "line_no":  line_no,
                    "type":     t,
                    **data,
    ...
    print(f"Total PAN matches    : {len(parsed)}")
